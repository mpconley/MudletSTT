...
describe("parsing an explicit phrase list", function()
  it("splits on semicolons and trims", function()
    local phrases, passes = test.parsePhraseList("score guild; guild score ;hair color")
    assert.same({ "score guild", "guild score", "hair color" }, phrases)
    assert.is_nil(passes)
  end)

  it("takes a trailing integer as the pass count", function()
    local phrases, passes = test.parsePhraseList("score guild; next level 3")
    assert.same({ "score guild", "next level" }, phrases)
    assert.equals(3, passes)
  end)

  it("returns an empty list for nothing", function()
    local phrases = test.parsePhraseList("")
    assert.same({}, phrases)
  end)
end)
